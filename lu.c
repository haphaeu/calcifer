...
// Varios codigos de algebra linear e matematica nesse site:
...
void main () {
    //const int N = 3;
    double A[N*N] = {4., 2., -3.,   -1., 3., 2.,   2., -8., 1.};
    double B[N]   = {8., 5., -3.};
    double x[N];
    int pivot[N];
    ...
    // Solving  A.x = B
    printf("\nSolving A.x = B for x:\n");
    Doolittle_LU_with_Pivoting_Solve(A, B, pivot, x, N);
    print_matrix(x, N, 1);
